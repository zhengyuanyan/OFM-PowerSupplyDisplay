...
/* clang-format off */
/* ---------------------------------------------------------------------------
 *  OFM-PowerSupplyDisplay 的 LVGL 配置
 *
 *  来源：LVGL 9.6.0 官方 lv_conf_template.h。对本模块的改动只有 4 类：
 *    1) 文件开头的 #if 0 -> #if 1（启用本文件）
 *    2) LV_USE_GENERIC_MIPI / LV_USE_ST7789 = 1（ST7789 驱动）
 *    3) LV_USE_FONT_COMPRESSED = 1（字库用 lv_font_conv 的默认压缩）
 *    4) LV_MEM_SIZE 64K -> 40K，并关闭所有用不到的部件/主题/布局
 *       （OBJ + LABEL + 默认主题就够画这个界面；app 分区只有 1 MB，
 *        全开部件会直接链接溢出 FLASH）
 *  找到本文件的方式：platformio.custom.ini 里定义了 LV_CONF_INCLUDE_SIMPLE
 *  并把本模块的 src/ 加进 include path。
 *  升级 LVGL 时请重新取模板并重做这几处改动。
 * ------------------------------------------------------------------------- */
...
/* OFM: RP2040 只有 264 KB RAM，界面很小，40 KB 足够（原模板 64 KB）。 */
...
/** Compressed fonts */
/* OFM: 生成的字库用 lv_font_conv 的默认 RLE 压缩（体积小很多），必须打开。 */
 ...
 *  Enable: LV_USE_GENERIC_MIPI
 */
/* OFM: 本模块用的就是 ST7789（1.54" 240x240）。 */
